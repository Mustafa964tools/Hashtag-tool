...
async function startServer() {
  ...
  app.post("/api/generate-hashtags", async (req, res) => {
    try {
      const { caption, platform = 'tiktok' } = req.body;
      if (!caption) {
      ...
      // Simple clean up in case the response includes markdown blocks
      const cleanHashtags = response.text ? response.text.replace(/`/g, '').trim() : '';

      res.json({ hashtags: cleanHashtags });
    } catch (error) {
      console.error("Error generating hashtags:", error);
      res.status(500).json({ error: "Failed to generate hashtags due to an internal error." });
    }
  });
